...
Il seguente filtro ha lo scopo di verificare l'autenticazione delle richieste HTTP che arrivano dall'esterno.  
L'autenticazione si basa su un **token JWT**, che viene analizzato per determinare se la richiesta può passare al backend e ai suoi servizi.
Il filtro richiede che il token sia presente nell'**header della richiesta**. Non è richiesta autenticazione solo nel caso in cui la destinazione della richiesta sia sugli endpoint backend legati all’**accesso o alla registrazione** nella piattaforma, poiché in quel caso l’utente non può essere ancora autenticato nel sistema.
Se invece l'autenticazione manca per altre destinazioni, viene restituito il codice **401 Unauthorized** e la richiesta viene scartata senza essere inoltrata ai servizi interni.
Per verificare l'autenticità del token, il gateway utilizza il **jwt secret**, una chiave segreta che permette di controllare che il token non sia stato modificato o generato da una fonte non autorizzata. Se la verifica ha esito positivo, il token viene considerato valido e il filtro estrae le **claims**, cioè le informazioni contenute nel JWT, come l'identificativo dell'utente, il ruolo o la data di scadenza.
Successivamente la richiesta viene inoltrata al backend.
In conclusione, il compito del filtro è verificare che tutte le richieste destinate ai servizi interni backend contengano un **token JWT valido**, garantendo che solo gli utenti autenticati possano accedere alle risorse del sistema.
